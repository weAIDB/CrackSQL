    ...
    for model_config in models_config:
        try:
            ...
            if 'deployment_type' not in model_config:
                logger.error(f"LLM model {model_config['name']} configuration missing required parameter: deployment_type")
                continue
...
def init_embedding_models(models_config):
    ...
    if not models_config:
        logger.error("Embedding model configuration not found, please check the EMBEDDING_MODELS section in the configuration file")
        return False
    ...
    for model_config in models_config:
        try:
            ...
            if 'deployment_type' not in model_config:
                logger.error(f"Embedding model {model_config['name']} configuration missing required parameter: deployment_type")
                continue
                
            if 'dimension' not in model_config:
                logger.error(f"Embedding model {model_config['name']} configuration missing required parameter: dimension")
                continue
...
def main():
    ...
    with app.app_context():
        ...
        if not (args.init_all or args.init_llm or args.init_embedding or args.init_kb):
            logger.error("Please specify an operation to perform, such as --init_all, --init_llm, --init_embedding, --init_kb, etc.")
            parser = argparse.ArgumentParser(description='Knowledge Base Initialization Script')
            parser.print_help()
            sys.exit(1)
